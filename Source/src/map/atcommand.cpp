 ...
 *------------------------------------------*/
ACMD_FUNC(security) {
	npc_event(sd, "SecurityManager::OnAtcommand", 0);
	return 0;
}
/*==========================================
 *
 ...
 *------------------------------------------*/
ACMD_FUNC(store) {
	npc_event(sd, "storage::OnMyStorage", 0);
	return 0;
}
/*==========================================
 *
 ...
 *------------------------------------------*/
ACMD_FUNC(gstore) {
	npc_event(sd, "storage::OnMyGuildStorage", 0);
	return 0;
}
...
ACMD_FUNC(accept)
{
	...
	//https://github.com/rathena/rathena/commit/a6818eb501d317f02d933165683236ecf45ce580
	//https://github.com/rathena/rathena/issues/2777
	if( !duel_check_player_limit( duel_get_duelid( sd->duel_invite ) ) )	// xMigue correccion realizada.
	{
		clif_displaymessage(fd, msg_txt(sd,351)); // "Duel: Limit of players is reached."
		return 0;
	}
...
#include "../custom/atcommand_def.inc"
		...
		ACMD_DEF(load),
		ACMD_DEF(speed),
		ACMD_DEF(security), // xMigue
		ACMD_DEF(store), 	// xMigue
		ACMD_DEF(gstore), 	// xMigue
		ACMD_DEF(storage),
